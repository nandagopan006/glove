...
class CartTestCase(TestCase):
    ...
    def test_add_to_cart_exceed_stock(self):
        ...
        self.client.post(
            reverse("add_to_cart"),
            {"variant_id": self.variant2.id, "quantity": 3},  # stock is 2
        )
        cart = Cart.objects.get(user=self.user)
    ...
    def test_update_cart(self):
        ...
        item = CartItem.objects.create(
            cart=cart, variant=self.variant, quantity=1
        )

        response = self.client.post(
            reverse("update_cart"), {"item_id": item.id, "quantity": 3}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_update_cart_exceed_stock(self):
        ...
        cart = Cart.objects.create(user=self.user)
        item = CartItem.objects.create(
            cart=cart, variant=self.variant2, quantity=1
        )

        self.client.post(
            reverse("update_cart"),
            {"item_id": item.id, "quantity": 3},  # stock is 2
        )
        item.refresh_from_db()
