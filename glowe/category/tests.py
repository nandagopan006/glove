...
class CategoryTests(TestCase):
    ...
    def test_add_category_view(self):
        response = self.client.post(
            reverse("add_category"), {"name": "Moisturizers"}
        )
    ...
    def test_toggle_category_status(self):
        response = self.client.post(
            reverse("toggle_category", args=[self.category.id])
        )
        self.assertRedirects(response, self.mgmt_url)
        self.category.refresh_from_db()
    ...
    def test_soft_delete_category(self):
        response = self.client.post(
            reverse("soft_delete_category", args=[self.category.id])
        )
        self.assertRedirects(response, self.mgmt_url)
        self.category.refresh_from_db()
    ...
    def test_restore_category(self):
        ...
        self.category.save()
        response = self.client.post(
            reverse("restore_category", args=[self.category.id])
        )
        self.assertRedirects(response, self.mgmt_url)
        self.category.refresh_from_db()
        self.assertFalse(self.category.is_deleted)
    ...
    def test_permanent_delete_category(self):
        ...
        self.category.save()
        response = self.client.post(
            reverse("permanent_delete_category", args=[self.category.id])
        )
        self.assertRedirects(response, self.mgmt_url)
