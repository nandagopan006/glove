...
def wishlist_page(request):
    ...
    wishlisted_product_ids = wishlist_items.values_list(
        "variant__product_id", flat=True
    )

    # get one variant per product (default variant or first) using distinct product  # noqa: E501
...
def move_to_cart(request, variant_id):
    ...
    if (
        cart_item
    ):  # if already increase the qty & and only if not morethan the maxqty

        if cart_item.quantity >= max_limit:  # if already have 5 qty to allow
            messages.warning(
                request,
                f"You can only add {max_limit} items . You Already Have The Max Quantity Items",  # noqa: E501
            )
            return redirect("wishlist")
