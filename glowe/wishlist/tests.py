...
class WishlistTestCase(TestCase):
    ...
    def test_toggle_wishlist(self):
        ...
        response = self.client.post(
            reverse("toggle_wishlist", args=[self.variant.id])
        )
        self.assertEqual(Wishlist.objects.count(), 1)
        self.assertRedirects(
            response, reverse("wishlist"), fetch_redirect_response=False
        )
    ...
    def test_remove_from_wishlist(self):
        ...
        self.client.post(
            reverse("remove_from_wishlist", args=[self.variant.id])
        )
        self.assertEqual(Wishlist.objects.count(), 0)

    def test_clear_wishlist(self):
        Wishlist.objects.create(user=self.user, variant=self.variant)
        self.client.post(reverse("clear_wishlist"))
        self.assertEqual(Wishlist.objects.count(), 0)

    def test_move_to_cart(self):
        Wishlist.objects.create(user=self.user, variant=self.variant)
        self.client.post(reverse("move_to_cart", args=[self.variant.id]))

        # Item should be in cart
        self.assertEqual(
            CartItem.objects.filter(
                cart__user=self.user, variant=self.variant
            ).count(),
            1,
        )
    ...
    def test_notify_me(self):
        ...
        self.client.post(reverse("notify_me", args=[self.variant.id]))
        self.assertEqual(
            StockNotification.objects.filter(
                user=self.user, variant=self.variant
            ).count(),
            1,
        )
