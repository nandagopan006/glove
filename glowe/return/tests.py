...
from wallet.models import Wallet  # noqa
...
class ReturnTestCase(TestCase):
    ...
    def setUp(self, mock_resize):
        ...
        self.product = Product.objects.create(
            name="Serum", slug="serum", category=self.category, is_active=True
        )
    ...
    def test_request_return_success(self):
        response = self.client.post(
            reverse("request_return", args=[self.item.id]),
            {
        ...
        self.assertRedirects(
            response, reverse("order_detail", args=[self.order.id])
        )
    ...
    def test_admin_approve_and_complete_return(self):
        ...
        # Schedule Pickup
        self.client.post(
            reverse("schedule_pickup", args=[r_req.id]),
            {
                "pickup_date": (timezone.now() + timedelta(days=1)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            },
        )
        r_req.refresh_from_db()
        self.assertEqual(
            r_req.return_status, ReturnRequest.Status.PICKUP_SCHEDULED
        )
