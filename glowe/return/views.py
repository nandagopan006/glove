...
def request_return(request, item_id):
    ...
    if (
        ...
        and timezone.now() > order.delivered_date + timedelta(days=7)
    ):
    ...
    if item.item_status in [
        ...
        messages.error(
            request,
            "This item has already been cancelled or a return has been requested.",  # noqa: E501
        )
        return redirect("order_detail", order_id=order.id)
    ...
    if request.method == "POST":
        ...
        if qty_to_return < 1 or qty_to_return > item.quantity:
            messages.error(
                request,
                f"Invalid quantity. You can return between 1 and {item.quantity} units.",  # noqa: E501
            )
            return redirect("request_return", item_id=item.id)
        ...
        for img in images:
            ReturnImage.objects.create(
                return_request=return_request, image=img
            )
    ...
    refund_ratio = (
        (discounted_subtotal / order.subtotal) if order.subtotal > 0 else 1
    )

    item.unit_refund = (item.price_at_time * refund_ratio).quantize(
        Decimal("0.01")
    )

    item.effective_refund = (item.unit_refund * item.quantity).quantize(
        Decimal("0.01")
    )
...
def request_full_return(request, order_id):
    ...
    if not active_items.exists():
        messages.error(
            request, "No eligible items found for return in this order."
        )
        return redirect("order_detail", order_id=order.id)
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            for item in active_items:
                ...
                # Copy images to each return request (if multiple items, they share the same proof)  # noqa: E501
                for img in images:
                    ReturnImage.objects.create(
                        return_request=return_request, image=img
                    )
        ...
        messages.success(
            request, "Full order return request submitted successfully."
        )
        return redirect("order_detail", order_id=order.id)
    ...
    for itm in active_items:
        itm.unit_refund = (itm.price_at_time * refund_ratio).quantize(
            Decimal("0.01")
        )
        itm.effective_refund = (itm.unit_refund * itm.quantity).quantize(
            Decimal("0.01")
        )
...
def admin_return_list(request):
    ...
    pending_count = ReturnRequest.objects.filter(
        return_status=ReturnRequest.Status.REQUESTED
    ).count()

    approved_today = ReturnRequest.objects.filter(
        return_status=ReturnRequest.Status.APPROVED, updated_at__date=today
    ).count()

    rejected_today = ReturnRequest.objects.filter(
        return_status=ReturnRequest.Status.REJECTED, updated_at__date=today
    ).count()
...
def admin_return_detail(request, return_id):
    ...
    if order.subtotal > 0:
        ...
        calculated_refund = (original_item_total * refund_ratio).quantize(
            Decimal("0.01")
        )
    else:
...
def approve_return(request, return_id):
    ...
    if r.return_status != ReturnRequest.Status.REQUESTED:
        ...
        messages.error(
            request, "This return cannot be approved at this stage."
        )
...
def schedule_pickup(request, return_id):
    ...
    r.pickup_date = request.POST.get("pickup_date") or (
...
def complete_return(request, return_id):
    ...
    if order.subtotal > 0:
        discounted_subtotal = order.subtotal - (order.discount_amount or 0)
        refund_ratio = discounted_subtotal / order.subtotal
        refund_amount = (original_item_total * refund_ratio).quantize(
            Decimal("0.01")
        )
    else:
    ...
    with transaction.atomic():
        ...
        returned_or_cancelled = all_items.filter(
            item_status__in=[
        ...
        if order.order_status in [
            ...
            OrderStatusHistory.objects.create(
                order=order, status=order.order_status, description=msg
            )
