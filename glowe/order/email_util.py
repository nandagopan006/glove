    ...
    for i, item in enumerate(order_items):
        ...
        if primary and primary.image:
            ...
            if os.path.exists(image_path):
                ...
                item.cid_id = cid_id
                attached_images.append({"path": image_path, "cid": cid_id})
            else:
                item.cid_id = None
        else:
            item.cid_id = None
...
def send_order_cancellation_email(
    request, order, cancelled_items=None, is_full_cancel=False, refund_amount=0
):
    """
    ...
    if is_full_cancel:
        ...
        items_to_show = order.items.select_related(
            "variant", "variant__product"
        ).prefetch_related("variant__product__images")
    else:
    ...
    html_content = render_to_string(
        "order/email/order_cancellation.html", context
    )
    text_content = strip_tags(html_content)

    subject = f"{'Order' if is_full_cancel else 'Item'} Cancelled - {order.order_number} ❌"  # noqa: E501
    from_email = settings.EMAIL_HOST_USER
    to_email = [order.user.email]
...
def send_order_delivered_email(request, order):
    ...
    html_content = render_to_string(
        "order/email/order_delivered.html", context
    )
    text_content = strip_tags(html_content)
