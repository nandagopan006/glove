...
class ProductTestCase(TestCase):
    ...
    def setUp(self, mock_resize):
        ...
        img = SimpleUploadedFile(
            "test_image.jpg", gif, content_type="image/jpeg"
        )
        self.image = ProductImage.objects.create(
            product=self.product, image=img, is_primary=True
        )
    ...
    def test_product_slug_auto_gen(self):
        ...
        p2 = Product.objects.create(
            name="Face Wash",
            category=self.category,
            description="Desc for product two",
        )
    ...
    def test_variant_sku_auto_gen(self):
        ...
        v2 = Variant.objects.create(
            product=self.product, size="100ml", price=250.00, stock=10
        )
    ...
    def test_product_detail_view(self):
        response = self.client.get(
            reverse("product_detail_view", args=[self.product.slug])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Face Wash")
    ...
    def test_add_product_submission(self):
        ...
        response = self.client.post(
            reverse("add_product"),
            {
                ...
                "description": "Description must be at least 12 characters long.",  # noqa: E501
        ...
        # Note: add_product view returns JSON or redirect depending on how it's called  # noqa: E501
    ...
    def test_variant_management_views(self):
        ...
        # View list
        response = self.client.get(
            reverse("variant_management", args=[self.product.id])
        )
        self.assertEqual(response.status_code, 200)
        # Add variant
        response = self.client.post(
            reverse("add_variant", args=[self.product.id]),
            {"size": "50ml", "price": 150.00, "stock": 100, "is_active": "on"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            Variant.objects.filter(product=self.product, size="50ml").exists()
        )
